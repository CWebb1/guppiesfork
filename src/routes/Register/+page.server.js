...
export const actions = {
	login: async ({ cookies, request }) => {
		...
		throw redirect(303, '/Dashboard'); // Send the user to the app once logged in
	},
    logout: async ({ cookies }) => {
        // Remove the token cookie to log the user out
        cookies.delete('token', { path: '/' });
        throw redirect(303, '/'); // And back to the home page
    },
    register: async ({ request }) => {
        ...
        throw redirect(303, '/Dashboard'); // Send the user to the login once registered
    }
};